...
#  def clear_all_truck_tags():
#     for sid in list(active_slot_ids):  # Copy the list to avoid runtime issues
#         if sid in truck_tags:
#             canvas.delete(truck_tags[sid])
#         if sid in warning_texts:
#             canvas.delete(warning_texts[sid])
...
def Warning_For_Outside_Park(tag_text,slot_id):
    ...
    if slot_id in truck_tags:           
        canvas.delete(print_icons[slot_id])
        del print_icons[slot_id]
        
    # canvas.delete(truck_tags[slot_id])
      #  del truck_tags[slot_id]
...
#         return
    ...
    if isWarningToday == "0":
        icon_id = canvas.create_image(
            1390.0, y_position - 20,
            image=print_icon_image,
            anchor="nw"
        )
        print_icons[slot_id] = icon_id
        
       # canvas.tag_bind(icon_id, "<Button-1>", lambda event, sid=slot_id, ttag=tag_text: Warning_For_Outside_Park(tag_text,sid))

        warning_text_id = canvas.create_text(
            1350, y_position,
            text=str(warningCount),
            font=("Arial", 14, "bold"),
            fill="gray" if warningCount > 0 else "gray",
            anchor="w"
        )
          ...
          #      print("Extracted data:",extracted_data)
                ...
                for item in extracted_data:
                    if(item["SlotId"] == None):
                        ...
                        if truck_tag:  
                            cheatpark_tag_number_text(slot_id, truck_tag, IsWarningToday, WarningCount)
